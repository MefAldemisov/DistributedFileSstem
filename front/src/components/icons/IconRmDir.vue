<script>
export default {
    name: "IconRmDir",
};
</script>
<template>
    <svg
        version="1.1"
        id="Capa_1"
        xmlns="http://www.w3.org/2000/svg"
        xmlns:xlink="http://www.w3.org/1999/xlink"
        x="0px"
        y="0px"
        viewBox="0 0 60 60"
        style="enable-background: new 0 0 60 60"
        xml:space="preserve"
    >
        <g>
            <path
                d="M47,30.5c-7.168,0-13,5.832-13,13s5.832,13,13,13s13-5.832,13-13S54.169,30.5,47,30.5z M47,54.5c-6.065,0-11-4.935-11-11
		s4.935-11,11-11s11,4.935,11,11S53.066,54.5,47,54.5z"
            />
            <path
                d="M51.95,38.551c-0.391-0.391-1.023-0.391-1.414,0L47,42.087l-3.536-3.536c-0.391-0.391-1.023-0.391-1.414,0
		s-0.391,1.023,0,1.414l3.536,3.536l-3.536,3.536c-0.391,0.391-0.391,1.023,0,1.414c0.195,0.195,0.451,0.293,0.707,0.293
		s0.512-0.098,0.707-0.293l3.536-3.536l3.535,3.535c0.195,0.195,0.451,0.293,0.707,0.293s0.512-0.098,0.707-0.293
		c0.391-0.391,0.391-1.023,0-1.414l-3.535-3.535l3.536-3.536C52.341,39.574,52.341,38.942,51.95,38.551z"
            />
            <path
                d="M31,43.5c0-8.822,7.178-16,16-16c5.356,0,10.094,2.655,13,6.708V19.5v-2v-7H26.515l-5-7H0v53h37.708
		C33.655,53.594,31,48.857,31,43.5z M58,12.5v5H31.515l-3.571-5H58z"
            />
        </g>
    </svg>
</template>